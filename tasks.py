...
def robot_rottentomatoes():
    ...
    try:
        ...
        if not movies_name:
            raise ValueError("No movies found in Excel file or Excel file is empty")
        
        all_results = []
        for movie_name in movies_name:
            # Process one movie at a time
            result = search_and_extract_movies(movie_name)  # returns a single dict
            insert_movies_to_db([result])  # Insert each movie immediately if you want
            all_results.append(result)   # Collect for Excel/email
        
        if not all_results:
            raise RuntimeError("No movie data was processed successfully")
            
        excel_path = save_to_excel(all_results)
        recipet_email = "sajinamatya88@gmail.com"
        send_reviews_excel_via_email(excel_path=excel_path, manual_recipients=recipet_email)
        
        print(f"Successfully processed {len(all_results)} movies and sent results via email")
        
    except Exception as e:
        print(f"Critical error in robot_rottentomatoes: {e}")
...
def get_movie_name_from_excel():
    ...
    try:
        ...
        if not movie_names:
            raise ValueError(f"No movies found in Excel file '{EXCEL_FILE_PATH}'. Make sure the file has a 'Movies' column with data.")
            
        print(f"Found {len(movie_names)} movies in Excel file: {movie_names}")
        return movie_names
    except FileNotFoundError:
        raise FileNotFoundError(f"Excel file '{EXCEL_FILE_PATH}' not found. Please ensure the file exists.")
    except Exception as e:
        raise RuntimeError(f"Error reading movie names from Excel file '{EXCEL_FILE_PATH}': {e}")


def search_and_extract_movies(movie_name):
    """
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        raise RuntimeError(f"Could not find title element for search result {i}")
                ...
                # Don't raise exception here as we want to continue with other results
                continue
        ...
        if exact_matches:
            try:
                ...
                # Don't raise exception here to allow processing of other movies
        else:
            print(f"No exact match found for '{movie_name}'")
            raise RuntimeWarning(f"No exact match found for movie '{movie_name}' in search results")
            
    except Exception as e:
        print(f"Error processing '{movie_name}': {e}")
        print("Continuing to next movie...")
        # Set error status but don't raise exception to allow other movies to be processed
        movie_dict['status'] = f'Error: {str(e)}'
    
    print(f"\nCompleted processing movie: '{movie_name}'")
    return movie_dict
...
def insert_movies_to_db(movie_data):
    server = os.environ.get("SERVER")
    database = os.environ.get("DATABASE")
    ...
    if not movie_data:
        raise ValueError("No movie data provided for database insertion")
    ...
    try:
        ...
        conn.commit()
        print(f"Successfully inserted {len(movie_data)} movie(s) to database")
    except pyodbc.Error as e:
        raise RuntimeError(f"Database insertion failed: {e}")
    except Exception as e:
        raise RuntimeError(f"Unexpected error during database insertion: {e}")
    finally:
        if conn:
            conn.close()
...
def save_to_excel(movie_data, excel_path=None):
    ...
    try:
        ...
        raise RuntimeError(f"Failed to create output directory '{output_folder}': {e}")
    ...
    try:
        ...
        print(f"Successfully saved {len(movie_data)} movies to Excel file: {excel_path}")
        return excel_path
    except Exception as e:
    ...
    try:
        if manual_recipients:
            if isinstance(manual_recipients, str):
                ...
                raise ValueError("Invalid manual_recipients format. Use string or list.")
        else:
            ...
            try:
                ...
                raise FileNotFoundError(f"Recipients Excel file '{recipients_excel}' not found")
            except Exception as e:
                raise RuntimeError(f"Error reading recipients from Excel file '{recipients_excel}': {e}")

        if not recipients:
            raise ValueError("No recipients found! Please provide recipients via manual_recipients or recipients Excel file.")
        ...
        if not all([smtp_server, smtp_user, smtp_password]):
            raise ValueError("Missing required SMTP environment variables: SMTP_SERVER, SMTP_USER, and/or SMTP_PASSWORD")
